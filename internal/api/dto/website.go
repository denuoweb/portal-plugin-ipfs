...
func (r HNSDomainRequest) Schema() *zog.StructSchema {
	return zog.Struct(zog.Shape{
		"Domain": zog.String().Required().Min(1).Max(255),
		"Mode":   zog.String().Optional().OneOf([]string{"", string(db.HNSModeDANE)}),
	})
}

func (r *HNSDomainRequest) ToModel() (*HNSDomainRequest, error) {
	if r.Mode == "" {
		r.Mode = string(db.HNSModeDANE)
	}
	return r, nil
}
...
type HNSDomainResponse struct {
	...
	CID                          string                `json:"cid"`
	Status                       string                `json:"status"`
	CustomerPublishFromHNSWallet db.HNSWalletBundle    `json:"customer_publish_from_hns_wallet"`
	PinnerManagedRecords         []db.HNSManagedRecord `json:"pinner_managed_records"`
	DNSSECDS                     string                `json:"dnssec_ds,omitempty"`
	TLSARecord                   string                `json:"tlsa_record,omitempty"`
	GatewayRoute                 db.HNSGatewayRoute    `json:"gateway_route"`
	GatewayRouteStatus           string                `json:"gateway_route_status"`
	Created                      time.Time             `json:"created"`
	Updated                      time.Time             `json:"updated"`
}

func (r *HNSDomainResponse) FromModel(model *db.HNSDomain) error {
	r.ID = model.ID
	r.WebsiteID = model.WebsiteID
	r.Domain = model.Domain
	r.DomainType = model.DomainType
	r.HNSMode = model.HNSMode
	r.Zone = model.DNSZoneName
	r.ZoneID = model.ZoneID
	r.CID = model.CID
	r.Status = model.Status
	r.DNSSECDS = model.DNSSECDS
	r.TLSARecord = model.TLSARecord
	r.GatewayRouteStatus = model.GatewayRouteStatus
	r.Created = model.CreatedAt
	r.Updated = model.UpdatedAt

	if len(model.HNSWalletBundle) > 0 {
		if err := json.Unmarshal(model.HNSWalletBundle, &r.CustomerPublishFromHNSWallet); err != nil {
			return err
		}
	}
	if len(model.PinnerManagedRecords) > 0 {
		if err := json.Unmarshal(model.PinnerManagedRecords, &r.PinnerManagedRecords); err != nil {
			return err
		}
	}
	if len(model.GatewayRoute) > 0 {
		if err := json.Unmarshal(model.GatewayRoute, &r.GatewayRoute); err != nil {
			return err
		}
	}
	return nil
...
var _ httputil.DTOValidator = (*HNSDomainRequest)(nil)
var _ httputil.DTORequest[*HNSDomainRequest] = (*HNSDomainRequest)(nil)
var _ httputil.DTOResponse[*db.HNSDomain] = (*HNSDomainResponse)(nil)
